    ...
    <div className="flex flex-col lg:flex-row gap-6 h-full">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 p-6">
          ...
          <form className="space-y-5" onSubmit={handleSchedule}>
            ...
            <div>
              ...
              <div className="flex flex-wrap gap-3">
                {PLATFORMS.map((p)=>{
                  ...
                  return (
                    <button key={p.id} type="button" onClick={()=>togglePlatform(p.id)} className={`flex itmes-center gap-1.5 p-3 rounded-md border transition-all duration-150 ${active ? "bg-red-50 border-red-300 text-red-500 scale-103":"border-slate-200 text-slate-500 hover:border-slate-300"}`}>
            ...
            <div>
              ...
              <textarea required rows={5} placeholder="What do you want to share today?" className="w-full px-5 py-4 bg-slate-50 border border-slate-200 rounded-2xl text-slate-900 text-sm placeholder-slate-400 outline-none resize-none" value={content}             
            ...
            <div>
              ...
              { mediaFile ? (
                <div className="relative rounded-xl overflow-hidden border border-slate-200 bg-slate-50">
                  ...
                  <button type="button" onClick={()=>{setMediaFile(null)}} className="absolute top-2 right-2 size-7 bg-slate-900/60 hover:bg-slate-900/80 text-white rounded-full flex itmes-center justify-center transition-colors">
      ...
      <div className="flex-1 flex flex-col gap-6 min-w-0">
        ...
        <div className="bg-white rounded-2xl border border-slate-200 overflow-hidden">
          <div className="flex itmes-center gap-2.5 px-5 py-4 border-b border-slate-100">
                ...
                scheduled.length === 0 ? (
                  ...
                  scheduled.map((post)=>(
                    <div key={post._id} className="px-5 py-4 hover:bg-slate-50/60 transition-colors">
                      <div className="flex itmes-center justify-between mb-2">
                        <div className="flex gap-1.5 itmes-center">
                        ...
                        <div className="flex items-center gap-2">
                          {post.mediaType && <span className="text-xs bg-slate-100 text-slate-600 border border-slate-200 px-1.5 py-0.5 rounded-md font-sembold capitalize">{post.mediaType}</span>}
