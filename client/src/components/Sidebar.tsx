...
const Sidebar = ({isOpen,setIsOpen}:{isOpen:boolean, setIsOpen:(val:boolean)=>void}) => {
    ...
    const navItems = [
        ...
        {name:"AI Composer",icon:UsersIcon, path:"/ai-composer"},
    ]
  return (
    <div className={`fixed inset-y-0 left-0 z-50 w-64 bg-white border-r border-slate-200 flex flex-col h-full transform transition-transform duration-200 ease-in-out md:relative md:translate-x-0 ${isOpen ? "translate-x-0" : "-translate-x-full"}`}>
       ...
       <div className='p-6 pb-4'>
            ...
            {navItems.map((item)=>{
                ...
                return (
                    ...
                    >
                        ...
                        {item.name}
                        {isActive && <span className='ml-auto w-[5px] h-5 rounded-full bg-red-500'></span>}
                    </NavLink>
                )
            })}
        </nav>
        ...
        <div className="p-4 border-t border-slate-100">
            <div className='flex items-center gap-3 p-2 rounded-xl hover:bg-slate-50  transion-colors'>
                <div >
